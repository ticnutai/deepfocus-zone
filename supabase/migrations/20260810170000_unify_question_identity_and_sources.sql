-- A signed-in user may use several Electron/browser installations. Device
-- mappings therefore cannot be one-to-one with a user account.
alter table public.offline_question_devices
  drop constraint if exists offline_question_devices_user_id_key;
create index if not exists offline_question_devices_user_idx
  on public.offline_question_devices(user_id);
...
begin
  ...
  -- Once a real cloud session exists, make it the authoritative owner for
  -- this installation. This joins browser/Electron submissions made by the
  -- same cloud account without merging unrelated local accounts by name.
