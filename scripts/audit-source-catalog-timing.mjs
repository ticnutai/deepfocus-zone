...
try{for(let i=0;i<3;i++){const start=performance.now();const {data,error,status}=await cloud.rpc('get_admin_question_source_tags');console.log(JSON.stringify({attempt:i+1,milliseconds:Math.round(performance.now()-start),status,rows:data?.length,error:error?{code:error.code,message:error.message}:null}));if(process.argv.includes('--shemesh')){console.log(JSON.stringify(data?.find(row=>row.source_id==='shemesh')));break;}}}finally{await cloud.auth.signOut();}
